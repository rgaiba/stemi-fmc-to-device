...
Nallamothu, Bates, Herrin, Wang, Bradley, and Krumholz (Circulation 2005;111:761) decomposed the time-to-balloon for STEMI transfer patients nationally using the NRMI-3/4 registry and quantified each component — door-in-door-out (DIDO), transport, and door-to-balloon at the receiving PCI center — for the first time. They reported a median total time-to-balloon of 180 minutes for transfer patients and identified DIDO (median ~64 minutes) as the largest single delay component. The paper is the foundational empirical reference for the Nallamothu decomposition that this analysis uses operationally.

The current analysis extends that framework geographically. Nallamothu et al. characterized the components — what is happening given the routing decisions EMS providers make. The contribution here is to identify where the routing decisions themselves are consequential: the subset of the U.S. population for whom EMS routing optimization, holding the hospital network constant, would meaningfully alter the Nallamothu decomposition. The two ask complementary questions. They asked: how long does each step take, and where are the bottlenecks. We ask: where in the country does the destination choice itself shift the decomposition meaningfully — and which hospital systems sit in those zones.
...
## Late-day scope decision (Paper 1 / Paper 2 split)

The work was descoped from a single combined-paper plan to a two-paper research program (see `pre_registration.md` Amendment 2026-05-07-A). Paper 1 — the AHA Scientific Sessions abstract and *Circulation: CVQO* submission — is now scoped to the traffic-aware drive-time geography of competitive PCI catchment zones, computed deterministically from public-source data with no D2B or DIDO prior assumptions. Paper 2 (forthcoming) integrates predictive models of hospital-level DIDO and D2B with the geographic substrate established in Paper 1, producing the per-patient ΔS2B estimates and system-level rankings. Paper 3 (forthcoming) is the prospective validation cohort. The DIDO/D2B operational documentation (`dido_d2b_operationalization.md`, `d2b_prior_plan.md`) remains in place as Paper 2 infrastructure.

The descope eliminates the largest single attack surface on Paper 1 (D2B prior assumptions) and produces a sharper claim — *here is the geographic substrate where institutional differences could plausibly determine optimal destination* — that stands defensibly on public-source drive-time data alone. The clinical magnitude framing shifts from "ΔS2B reduced by ≥15 min in N patients" (which depended on D2B priors) to "N STEMI patients per year reside in zones where the optimal routing destination cannot be determined by proximity alone" (which depends only on the deterministic drive-time matrix).
